...
async function getDadosOntem(start, end) {
    ...
    dados2 = normalizeData(dados2.data, 300, 1100, nomeVariavelBaseDados);
...
async function getDadosMinutos() {
    ...
    if (x) {
        ...
        dados = normalizeData(dados.data, 300, 1100, nomeVariavelBaseDados);
        ...
        refreshOtherTowers(infosVariaveisByName, nomeTorreAtual, 5, grandeza, nomeVariavelBaseDados, 300, 1100);
        gauge = new JustGage({
            id: "gauge",
            value: 0,
            defaults: defs1
...
async function getDadosData(start, end) {
    ...
    if (x) {
        ...
        distime();
        refreshOtherTowersByDate(infosVariaveisByName, nomeTorreAtual, dateStartNorm.toISOString(), dateEndNorm.toISOString(), grandeza, nomeVariavelBaseDados, 300, 1100);
...
async function refreshConteudo() {
    ...
    if (x) {
        ...
        graficos(variavel);
        ResetZoom();
        createTable(dados, variavel, "tabelaEstatis", grandeza); 3
        createTableShowData(dados, "tabelaMostrarDados", grandeza)
        checkData(dados, chartjs, comIntervalo, intervalo, tempoMinutos);
        dadosNormaOntem = await getDadosOntem(dateStart, dateFinish);
        if (linhaOntem) {
